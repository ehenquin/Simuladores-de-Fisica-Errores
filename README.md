# Simuladores de Física - Errores Experimentales 2026

Una plataforma interactiva para comprender cómo se mide, cómo se estima una magnitud física y cómo se propagan los errores experimentales.

## Características
- **Teoría y fórmulas**: Conceptos fundamentales de error absoluto, relativo, promedio, desviación estándar, incertidumbre y propagación.
- **Caída libre con cronómetro manual**: El alumno mide manualmente el tiempo de caída de una esfera y observa su propio error experimental.
- **Balanza de brazos**: Simulación dinámica de una balanza en equilibrio mediante pesas, torque y comparación de masas.
- **Propagación de errores**: Áreas, volúmenes y magnitudes derivadas calculadas a partir de mediciones con incertidumbre.

## Autor
Diseñado por **Dr. Eduardo R. Henquín** | Basado en simuladores educativos originales (2005)
[LinkTree Profesional](https://ehenquin.github.io/LinkTreePro/)
